...
class ImageGraph:
    ...
    def print_adjacency_matrix(self):
        ...
        mat = [[0 for i in range(len(self.nodes))] for j in range(len(self.nodes))]

        for node in self.nodes:
            for edge in node.edges:
                mat[node.index][edge] = 1
    ...
    def bfs(self, start_index, color):
        ...
        node_queue = Queue() #a queue of the nodes to be visited
        self.nodes[start_index].visit_and_set_color(color) #set the initial node to the desired color
        node_queue.enqueue(self.nodes[start_index]) #add the initial node to the queue
        
        while not node_queue.is_empty():
            color_node = node_queue.dequeue()
            
            for i in color_node.edges:
                node = self.nodes[i]
                if node not in visited and node.color == color_node.prev_color:
